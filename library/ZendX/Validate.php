...
->rules('Email', array(
	array('not_empty'),
	array('email'),	array('min_length',	array(':value',	6)),
	array('max_length',array(':value',30)),
	array('regex',array(':value','/[a-z0-9._]/i'))))
->rules('Password', array(
	array('not_empty'),array('password'),
	array('min_length',array(':value',6)),
	array('max_length',array(':value',16))));
$rs = $post->check();
if (! $rs)
{
	$errors = $post->errors('form.login');
}
 */
/**
 *  表单验证
 ...
 *
 */
class ZendX_Validate implements ArrayAccess
{
	/**
	 *
	...
	{
		return new ZendX_Validate($array);
	}
	...
	*/
	...
	public function rules ($field ,array $rules)
	{
		...
		{
			$this->rule($field, $rule[0], ZendX_Array::get($rule, 1));
		}
		return $this;
	}
	...
	public function check ()
	{
		...
		$expected = ZendX_Array::merge(array_keys($original), array_keys($this->_labels));
		// Import the rules locally
		$rules = $this->_rules;
		foreach ($expected as $field)
		{
			// Use the submitted value or NULL if no data exists
			$data[$field] = ZendX_Array::get($this, $field);
			if (isset($rules[TRUE]))
			{
		...
		foreach ($rules as $field => $set)
		{
			...
			foreach ($set as $array)
			{
				...
				elseif (method_exists('ZendX_Valid', $rule))
				{
					// Use a method in this object
					$method = new ReflectionMethod('ZendX_Valid', $rule);
				...
				if (! in_array($rule, $this->_empty_rules) and ! ZendX_Valid::not_empty($value)) continue;
	 ...
	 * @return  array
	 */
	public function errors ($file = NULL ,$lang = NULL)
	{
		if ($file === NULL)
		{
		...
		foreach ($this->_errors as $field => $set)
		{
			...
			$label2 = self::path($file,"{$field}.label", $lang);
			if($label2)$label=$label2;
			// Start the translation values list
			$values = array(
					':field' => $label,
					':value' => ZendX_Array::get($this, $field)
			);
			if (is_array($values[':value']))
			{
				// All values must be strings
				$values[':value'] = implode(', ', ZendX_Array::flatten($values[':value']));
			}
			if ($params)
			{
				foreach ($params as $key => $value)
				{
					...
					{
						// All values must be strings
						$value = implode(', ', ZendX_Array::flatten($value));
					}
			...
			$find_arr=array(
					array($file,"{$field}.{$error}"),// Found a message for this field and error
					array($file,"{$field}.default"),// Found a default message for this field
					array($file,$error),// Found a default message for this error
					array('validation',$error),// Found a default message for this error
			);
			foreach ($find_arr as $v)
			{
				$message = self::path($v[0],$v[1],$lang);
				if($message)break;
			}
			// No message exists, display the path expected
			if(!$message)$message = "{$file}.{$field}.{$error}";
	...
	public static function load ($_pack = 'common' ,$_lang = NULL)
	{
		...
		$table = array();
		$path = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR .  "{$_lang}" . DIRECTORY_SEPARATOR .  str_replace(array(
				'.'
	...
	public static function path ($pack ,$key ,$lang = NULL)
	{
		...
		return ZendX_Array::path($arr, $key, NULL);
	}
