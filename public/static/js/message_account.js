$(document).ready(function(){
	...
	$('#change_tab').click(function(e){
		...
		if(id =='official') {
			 location.href= "/operation/account/index";
		} else if(id == 'enterprise') {
			 location.href= "/operation/account/enterprise";
		}
	});
	 ...
	 $('#check_form').validate({
		   ...
		   submitHandler:function() {
			   ...
			   var pass = $('input[type=radio]').filter(':checked').val();
				$.post('/operation/account/check/id/'+$('#message_id').val(), param, function(result){
					if(result.status == 200) {
						$('#message_dialog .message_text').text('保存成功！');
						$( "#message_dialog" ).dialog( "open" );
						$('#close_message').unbind('click');
						$('#close_message').click(function(){
							location.href="/operation/account/index?s="+Math.random();
						});
   ...
   //产品选择
	 $('#select_product').click(function(){
		 $( "#product_list_div").load('/operation/account/products', function(){
	 ...
	 $('#test_msg_form').validate({
		   ...
		   submitHandler:function() {
			   ...
			   $.post('/operation/account/checktestuser', param, function(result){
					if(result.status == 200) {
						$('#test_msg_form').hide();
						...
						$.post('/operation/account/dotesttrans',param_form, function(response){
		...
		$( "#confirm_dialog" ).dialog({
            ...
            buttons: {
                ...
                "确定": function() {
                	if(msg_id) {
	                	$.post('/operation/account/delete/', {id:msg_id}, function(e) {
                            location.reload();
                            // location.href="/operation/account/index?s="+Math.random();
	        			},'json');
                    }
		...
		$('.push_msg').click(function(e){
			...
			msg_id = id;
            $.post('/operation/account/push/', {id:msg_id}, function(e) {
               location.reload();
            },'json');
            return false;
		});
