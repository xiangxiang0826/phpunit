 ...
 *
 */
class Model_TemplateType extends ZendX_Model_Base {
	protected $_schema = 'cloud';
	protected $_table = 'EZ_TEMPLATE_TYPE';
	...
	*/
	public function delete($id) {
		$sql = "SELECT id FROM EZ_TEMPLATE WHERE template_type_id = ? LIMIT 1";
		$row = $this->get_db()->fetchRow($sql, $id);
		if($row) return false;
		$sql = "DELETE  FROM {$this->_table} WHERE id = ?";
		return $this->get_db()->query($sql, $id);
	}
}