 ...
 */
class Operation_CustomController extends ZendX_Controller_Action
{
	/**
	 * 官方推送消息列表
	 */
	public function indexAction(){
		...
		$select = $mesageModel->createCondition($condition, $type='custom');
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()) {
            ...
            $validateOriginalEx = $this->validateOriginalMessageEx($data['original_message']);
			if($validate === true && $validateOriginal === true && $validateOriginalEx === true) {
                ...
                $originalMessage = array(
                    'action' => $data['original_message']['text'],
                    'ex' => $ex
                );
                $offlineValue = '';
                $offlineUnit = '';
	...
	public function deleteAction(){
		...
		if($messageId) {
			...
			if($result['enterprise_id'] == Model_Enterprise::DEFAULT_EZ_ID) {
                //删除状态监测的条件
...
$post = ZendX_Validate::factory($data)->labels(array(
	 ...
	 * @return boolean|Ambigous <multitype:, multitype:string >
	 */
	protected  function validateOriginalMessageEx($data){
        return true;
	}
	...
	public function dotesttransAction(){	
		if($this->getRequest()->isPost()) {
			...
			if($validate === true && $validateOriginal === true && $validateOriginalEx === true) {
                ...
                // print_r(json_encode($postData));exit('.ln='.__line__);
                $errorCode = '';
                $errorContent = '';
                $pushMessageId = '';
                $status = Model_PushMessage::STATUS_PENDING;
	...
	public function needcheckAction(){
		...
		$number = $message->getNumberBystatus(Model_PushMessage::STATUS_PENDING, 'custom', 1);
		if($number) {
