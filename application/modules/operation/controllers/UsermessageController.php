 ...
 * @version $v1 2014/11/22 10:40$
 */
class Operation_UsermessageController extends ZendX_Controller_Action
{
	...
	public function indexAction(){
		...
		$select = $mesageModel->createCondition($condition, $type='notice');
	...
	public function enterpriseAction(){
        ...
        $listStatus = $mesageModel->getListStatusMap();
        $publishedStatus = $mesageModel->getSendingStatusMap();
		$select = $mesageModel->createConditionForEnterprise($condition, $type='notice');
		$paginator = Zend_Paginator::factory($select);
		$paginator->setItemCountPerPage($this->page_size);// 测试分页 =1
		$paginator->setCurrentPageNumber($page);
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()) {
            ...
            $validateOriginalEx = $this->validateOriginalMessageEx($data['original_message']['ex'], $data['original_message']['go']);
			if($validate === true && $validateOriginal === true && $validateOriginalEx === true) {
                ...
                $expireTime = '';
                if($data['is_offline']){
                    if(isset($data['expire_time']) && isset($data['expire_time']['value']) && isset($data['expire_time']['unit'])){
                        $unit = 3600; 
                        if($data['expire_time']['unit'] == 'day'){
                            $unit = 24*3600; 
                        }
                        $interval = $data['expire_time']['value']*$unit;
                        $expireTime = date('Y-m-d H:i:s', time()+$interval);
                    }
                }
                $dateNow = date('Y-m-d H:i:s');
                // $ex = array();
                ...
                if($go && $go != 'app'  && $go != 'none'){
                    ...
                    if($go == 'activity'){
                        $key = $go.'_android';
                        $exItem[$key] = $data['original_message']['ex'][$key];
                        $ex[] =  $exItem;
                        $exItem = array();
                        $key = $go.'_ios';
                        $exItem[$key] = $data['original_message']['ex'][$key];
                        ...
                        $exItem[$go] = $data['original_message']['ex'][$go];
                        $ex[] =  $exItem;
                    }
                }
                ...
                $offlineUnit = '';
                if($data['is_offline']){
                    $offlineValue = $data['expire_time']['value'];
                    $offlineUnit = $data['expire_time']['unit'];
                }
                $postData = array(
	...
	protected  function validateOriginalMessage($data){
		...
		$post->rules('go', array(
                               array('in_array', 
                                        array(
                                            ':value', 
                                            array('none','app','activity','url','custom_go')
                                        )
                               )
                        )
        );
	 ...
	 * @return boolean|Ambigous <multitype:, multitype:string >
	 */
	protected  function validateOriginalMessageEx($data, $goType){
        if($goType != 'app' && $goType != 'none'){
            switch($goType){
                case 'activity':{
                    if(!(isset($data[$goType.'_android'])  && isset($data[$goType.'_ios']))){
                        if($data[$goType.'_android'] == '' && $data[$goType.'_ios']  == ''){
                            return false;
                        }
                    }
                    break;
                ...
                default:{
                    if(!isset($data[$goType]) || $data[$goType] == ''){
                        return false;
                    }
                    break;
                }
	...
	public function productsAction() {
		...
		echo $this->view->render('usermessage_products.phtml'); 
	}
	...
	public function appsAction(){
        ...
        $this->view->base_url .= (!strpos($this->view->base_url, 'selectmodel='))?('&selectmodel='.$selectModel):'';
        $this->view->select_model = $selectModel;
		echo $this->view->render('usermessage_apps.phtml'); 
	}
	...
	public function platformsAction(){
		...
		echo $this->view->render('usermessage_platforms.phtml'); 
	}
	...
	public function dotesttransAction(){	
		if($this->getRequest()->isPost()) {
			...
			if($validate === true && $validateOriginal === true && $validateOriginalEx === true) {
                ...
                if(isset($content['ex'])){
                    $content['ex'][] = array('id' => time());
                    $content['extend'] = $content['ex'];
                    unset($content['ex']);
                }else{
                    $content['extend'] = array( array('id' => time()) );
                }
                ...
                $postData = array(
                    ...
                    'push_type' => $pushTypeArr[$data['user_type']],
                    'push_rule' => $pushRule,
                    'is_appbox' => (int)$data['is_appbox'],
                    'is_accountbox' => 0,
                    'send_type' => $data['send_type'],
                    'start_time' => $startTime,
                    'expire_time' => $expireTime,
                    'is_offline' => (int)$data['is_offline'],
                    'is_test' => 1,
                    'content' => $content
                );
	...
	public function needcheckAction(){
		...
		$number = $message->getNumberBystatus(Model_PushMessage::STATUS_PENDING, $messageType='notice', 0);
		if($number) {
